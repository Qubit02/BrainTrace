...
class TestMemoIntegration:
    ...
    def _signal_handler(self, signum, frame):
        ...
        print(f"\n시그널 {signum} 수신. Memo 테스트 데이터 정리 중...")
        self.cleanup()
        sys.exit(1)
    ...
    def test_memo_creation(self):
        ...
        try:
            ...
            response = requests.post(f"{BASE_URL}/memos/", json=memo_data, timeout=30)
    ...
    def test_memo_content_retrieval(self):
        ...
        try:
            ...
            # 메모 내용 조회
            response = requests.get(f"{BASE_URL}/memos/{self.created_memo_id}", timeout=30)
            assert response.status_code == 200
    ...
    def test_memo_update(self):
        ...
        try:
            ...
            response = requests.put(f"{BASE_URL}/memos/{self.created_memo_id}", json=update_data, timeout=30)
