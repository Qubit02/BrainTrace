...
export default function SourcePanel({
  ...
  useEffect(() => {
    if (focusSource) {
      const targetFile = allFiles.find(f => String(TYPE_META[f.type]?.id(f)) === String(focusSource.id));
      if (targetFile) {
          ...
          <div>
            ...
            {!openedFile && (
              <div className="action-buttons">
                {/* 소스 추가 버튼 (아이콘/텍스트 토글) */}
                <button
                  className={`pill-button ${panelWidth < RESPONSIVE_THRESHOLDS.SOURCE ? 'icon-only' : ''}`}
                ...
                <button
                  className={`pill-button${showSearchInput ? ' active' : ''} ${panelWidth < RESPONSIVE_THRESHOLDS.SEARCH ? 'icon-only' : ''}`}
